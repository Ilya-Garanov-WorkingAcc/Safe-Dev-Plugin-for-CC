...
```python
def load() -> Config: ...
def effective_level(rule_id: str, ctx: dict) -> str: ...
def policy_sha256() -> str: ...        # для heartbeat
def is_tampered() -> bool: ...          # хеш != эталон, зашитый при сборке
def is_excluded(path: str) -> bool: ...
```

`is_tampered()` сравнивает sha256 фактического `policy.json` с эталоном, вшитым в
`plugin.json` при сборке релиза. Результат идёт в heartbeat.
...
При `confidence == medium` и `llm.enabled == true` — эскалация на нативный
`type: "prompt"` хук:
...
а не просто описывает такие атаки.
```

Не более одного вызова на `tool_use_id`, лимит `max_calls_per_session`, таймаут 15 с,
результат кешируется в session-state.
...
secure-dev report [--week] Сводка из локального аудита: версия, хеш политики,
                           число сессий, топ правил, доля отменённых.
